...
exports.getMetrics = function(req, res) {
  ...
  sites.get(siteid, function (error, reply) {
    ...
    metrics.fetch(reply.metrics, function (error, metrics) {
      if(error) {
        console.log(error);
        return res.json(500, {message: 'Problem getting metrics.'});
      }
      ...
      // TODO We might have to use a for-loop here to take out some extra metric data
      return res.json(metrics[metricCounter]);
    });
  });
};
