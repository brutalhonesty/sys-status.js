...
function _uploadFileImage(siteid, image, uploadDir, relativeDir, imageType, res) {
  ...
  if(imageType === 'favicon') {
    ...
    relativePath = relativeDir + '/favicon/' + imageName;
  } else if(imageType === 'logo') {
    //Max 5 MB, min 1 KB
    if(imageSize > 5242880 || imageSize < 1024) {
      fileValid = false;
    ...
    relativePath = relativeDir + '/logo/' + imageName;
  } else {
    //Max 5 MB, min 1 KB
    if(imageSize > 5242880 || imageSize < 1024) {
      fileValid = false;
    ...
    relativePath = relativeDir + '/cover/' + imageName;
  }
  if(fileValid) {
