  ...
  if(fileValid) {
    ...
    fs.rename(imagePath, targetPath, function(err) {
      ...
      sites.get(siteid, function (error, reply) {
        ...
        // Store new data in DB
        sites.set(reply, siteid, function (error) {
          if(error) {
            console.log(error);
            return res.json(500, {message: 'Problem registered image.'});
          }
...
exports.uploadLogo = function(req, res) {
  ...
  if(validator.isURL(imageURL)) {
    return _uploadURLImage(siteid, imageURL, req.uploadDir, 'logo', res);
  } else {
    return _uploadFileImage(siteid, image, req.uploadDir, 'logo', res);
  }
};
