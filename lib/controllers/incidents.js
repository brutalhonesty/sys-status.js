...
exports.getIncidents = function(req, res) {
  ...
  sites.get(siteid, function (error, reply) {
    ...
    for(var incidentCounter = 0; incidentCounter < reply.incidents.length; incidentCounter++) {
      ...
      // if thirtydays + last updated time is after today and its completed, purge incident from response
      if(updateThreshold.isBefore(moment.utc()) && reply.incidents[incidentCounter].completedTime !== null) {
...
// Incidents which are deleted are truely deleted. Get incidents returns the last 30 days worth of incidents and any which are not closed yet.
...
exports.getIncidentHistory = function(req, res) {
  ...
  sites.view('sites', 'by_url', {reduce: false, startkey: siteurl, endkey: siteurl + '\u9999'}, function (error, reply) {
    ...
    var siteName = reply.rows[0].value.siteName;
    var incidents = reply.rows[0].value.incidents;
    var maintenances = reply.rows[0].value.maintenance;
    var history = incidents.concat(maintenances);
    if(history.length === 0) {
      return res.json({history: [], siteName: siteName});
    }
    return res.json({history: history, siteName: siteName});
  });
};
