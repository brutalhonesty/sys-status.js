...
function _parseFixes(prefix, options, callback) {
  if(typeof(prefix) !== 'string') {
    return callback('Prefix needs to be a string.');
  }
  ...
  if(prefix[0] !== '<') {
    ...
    switch(prefix.substring(1, prefix.length - 1)) {
      case 'status':
        return callback(null, options.status);
      case 'siteurl':
        return callback(null, options.siteurl);
      default:
        return callback('Invalid prefix option.');
    }
  }
}
      ...
      if(reply.twitter.allowTweets) {
        ...
        twitter.create(function (error) {
          ...
          var options = {
            ...
            siteurl: url.resolve(reply.siteName, '/statusPage')
          };
